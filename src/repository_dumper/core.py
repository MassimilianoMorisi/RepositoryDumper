...
#FileValidator = Callable[[str], bool]
...
#NodeVisitor = Callable[[DirectoryNode], None]
...
class DirectoryWalker:
    ...
    def __get_directory_tree_recursively(self, path: str) -> Union[DirectoryNode, None]:
        ...
        return DirectoryNode(directory_name, path, is_file, sub_items)
...
class ExportWriter:
    ...
    def __extract_path_from_root(cls, root_dir_name: str, absolute_path: str) -> str:
        ...
        try:
            start_index = path.parts.index(root_dir_name)

        except ValueError:
            raise ValueError(f"Root '{root_dir_name}' not found in path: {absolute_path}")
    ...
    def __get_formatted_block(cls, root_dir_name: str, filepath: str) -> Union[str, None]:
        ...
        content: str = f"{cls.__BORDER}\n"
        content += f"{cls.__extract_path_from_root(root_dir_name, filepath)}\n"
        with open(filepath, "rt") as file:  
            content += file.read()
        content += f"{cls.__BORDER}\n"
